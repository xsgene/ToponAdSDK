...
@interface TurboAdSplashManager ()
@property (nonatomic, strong) TurboAdThreadSafeDictionary *adCache;
@end

@implementation TurboAdSplashManager

+ (instancetype)sharedManager {
    static TurboAdSplashManager *instance = nil;
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _adCache = [[TurboAdThreadSafeDictionary alloc] initWithQueueLabel:@"com.turboadsdk.splash.cache"];
    }
    return self;
}
...
- (void)clearCache {
    ...
    TurboLogInfo(@"Splash cache cleared");
}
