...
@interface TurboAdNativeManager ()
@property (nonatomic, strong) TurboAdThreadSafeDictionary *adCache;
@end

@implementation TurboAdNativeManager

+ (instancetype)sharedManager {
    static TurboAdNativeManager *instance = nil;
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _adCache = [[TurboAdThreadSafeDictionary alloc] initWithQueueLabel:@"com.turboadsdk.native.cache"];
    }
    return self;
}
...
- (void)addAdWithPlacementID:(NSString *)placementID adObject:(TurboAdObject *)adObject {
    ...
    TurboLogDebug(@"Native ad added for: %@, total: %lu", placementID, (unsigned long)ads.count);
}
...
- (void)clearCache {
    ...
    TurboLogInfo(@"Native cache cleared");
}
